...
public class BossController : MonoBehaviour
{
    ...
    [Header("Time Line Boss Start")]
    [SerializeField] private GameObject CountBattleBoss;
    ...
    private void Awake()
    {
        CountBattleBoss.SetActive(false);
    }
    private void Start()
    {
    ...
    private void Update()
    {
        if (TimeLine > _overTime)
        {
            CountBattleBoss.SetActive(true);
            Animator.SetBool("Moving", false);
            TimeLine -= Time.deltaTime *2;
            Count.text = Mathf.Round(TimeLine).ToString();
